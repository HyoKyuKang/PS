...
vector<ll> convolution_and(vector<ll> a, vector<ll> b) {
    fwht_and(a, false); //and or xor만 수정
    fwht_and(b, false); //and or xor만 수정
    for(ll i=0;i<(1ll<<n);i++) a[i]*=b[i];
    fwht_and(a, true);  //and or xor만 수정
    return a;
}
